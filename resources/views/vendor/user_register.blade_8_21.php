	...
	<table cellpadding="0" cellspacing="0">
        ...
        <div class="form-group" ng-class="{ 'has-error': form.Type.$dirty && form.Type.$error.required }">
            <table width="100%">
                <tr>
                    ...
                    <td width="50%">                         
                        <select name="Type" id="Type" class="form-control" ng-model="user.Type">                            
                            @if ($users_info->media_type === "m")
                            <option value="m"    selected   >Media owner</option>
                            @else
                            <option value="m">Media owner</option>
                            @endif
                            @if ($users_info->media_type === "s")
                            <option value="s" selected >Service provider</option>
                            @else
            ...
            <div class="form-group" ng-class="{ 'has-error': form.mobileNumber.$dirty && form.mobileNumber.$error.required }">
                <table width="100%">
                    <tr>
                        ...
                        <td><input type="tel" name="mobileNumber" pattern="[7-9]{1}[0-9]{9}" id="mobileNumber" class="form-control"  value="{{ $users_info->land_phone }}"/>
            ...
            <div class="form-group" ng-class="{ 'has-error': form.company_name.$dirty && form.company_name.$error.required }">
                <table width="100%">
                    <tr>
                        ...
                        <td width="50%">
                                        <input value="{{ $users_info->company_name }}" type="text" name="company_name" id="company_name" class="form-control" />
            ...
            <div class="form-group" ng-class="{ 'has-error': form.Co_Website.$dirty && form.Co_Website.$error.required }">		
                <table width="100%">
                    <tr>
                        ...
                        <td width="50%">
                                <input type="url" name="Co_Website" id="Co_Website" class="form-control" value="{{ $users_info->website }}" />
            ...
            <div class="form-group" ng-class="{ 'has-error': form.co_email_id.$dirty && form.co_email_id.$error.required }">
                <table width="100%">
                    <tr>
                        ...
                        <td width="50%">
                            <input type="email" name="co_email_id" id="co_email_id" class="form-control" value="{{ $users_info->off_emailId }}" />
		...
		<div class="form-group" ng-class="{ 'has-error': form.Landline_Ph_No.$dirty && form.Landline_Ph_No.$error.required }">
                    ...
                    <tr>
                        ...
                        <td width="50%">
                            <input type="number" maxlength="12" name="Landline_Ph_No" id="Landline_Ph_No" class="form-control" value="{{ $users_info->mobile_number }}" />
                        ...
                        <td width="50%">
                            <select name="Location" id="Location" class="form-control">                          
                                @foreach($posts as $post)          
                                    @if($users_info->location === $post->id)
                                    <option value='{{ $post->id }}' selected>{{ $post->country_name }}</option>
                                    @else
		...
		<div class="form-group" ng-class="{ 'has-error': form.current_office_status.$dirty && form.current_office_status.$error.required }">
                    ...
                    <tr>
                        ...
                        <td width="50%">
                            <select name="current_office_status" id="current_office_status" class="form-control">
                                @if($users_info->current_off_status === 1)
                                    <option value="1" selected>Head office</option>
                                @else
                                    <option value="1">Head office</option>
                                @endif
                                
                                @if($users_info->current_off_status === 2)
                                    <option value="2" selected>Branch office</option>
                                @else
                                    <option value="2">Branch office</option>
                                 @endif
                                 
                                 @if($users_info->current_off_status === 3)
                                    <option value="3" selected>regional office</option>
                                @else
		...
		<div class="form-group" ng-class="{ 'has-error': form.Address.$dirty && form.Address.$error.required }">
                    ...
                    <tr>
                        ...
                        <td width="50%">    
                            <textarea rows="5" cols="25" name="Address" id="Address" class="form-control"> {{ $users_info->current_address }} </textarea>                                
...
{
        /*var $var = $.param($scope.user);
        alert($var);
        return ralse;*/
    $scope.register = function() {                        
            $http({
                url: "/laravel_new/public/user_register_submit",
                method: "POST",
